...
This document summarizes the full troubleshooting process of attempting to integrate WireGuard VPN access to the CSL lab from an external Windows client, through OPNsense firewall with VLANs and routed internal lab networks.
...
The CSL Lab now runs without dependency on WireGuard for Windows, and all lab access functions reliably via the internal VPN tunnel.
